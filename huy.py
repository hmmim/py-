...
ctda_page = requests.get("https://nus.edu.sg/")
...
with open('NEWS.md', 'w', encoding='utf-8') as f:
    f.write(f'# All news\n_Last update: **{datetime.now(tz=ZoneInfo("Asia/Singapore"))}**_\n## CS\n')
    for i, section in enumerate(ctda_sections):
        f.write(f'### {ctda_section_titles[i]}\n')
        news = [[el.contents[0].text, el.contents[0].attrs['href'], el.contents[-1].text] for el in
                section.find_all(class_='listing-item')]
        for n in news:
    ...
    for news in fit_news_raw:
        ...
        f.write(f' - {day}-{month}-{year}: [{title}](https://nus.edu.sg/{href})\n')
    ...
    for i in range(len(news_dates)):
        ...
        f.write(f' - {news_dates[i]}: [{news_titles[i]}](https://nus.edu.sg/{news_links[i]})\n')
